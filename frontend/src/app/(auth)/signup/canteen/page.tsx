...
export default function CanteenSignupPage() {
  ...
  const onSubmit = async (data: SignupForm) => {
    setIsLoading(true);
    try {
      // Hardcode role to canteen
      const payload = { ...data, role: "canteen" as const };
      const { message, user } = await authApi.register(payload);
      setAuth(user);
      toast.success("Akun Mitra Kantin berhasil dibuat!");
      // Directly route to create canteen page
      router.push("/canteen/create");
    } catch (error: unknown) {
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-muted/40 p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardDescription>Mendaftar sebagai Mitra Kantin</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input id="email" type="email" placeholder="kantin@campus.ac.id" {...register("email")} />
            ...
            <div className="flex flex-col gap-2 mt-4 text-center text-sm">
              ...
              <p className="text-muted-foreground">
                Hanya ingin pesan makanan?{" "}
                <Link href="/signup" className="text-primary hover:underline font-medium">Daftar sebagai Pelanggan</Link>
              </p>
            </div>
          </form>
        </CardContent>
